    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(32,165,110,0.14),_transparent_26%),linear-gradient(180deg,_#f8fafc,_#ffffff)] px-4 py-12 dark:bg-slate-950">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="glass-panel rounded-[2rem] p-8">
          <p className="text-xs uppercase tracking-[0.45em] text-clinic-600">AI-Powered Care</p>
          <h1 className="mt-4 text-4xl font-semibold text-slate-900 dark:text-white">{isLogin ? 'Welcome back' : 'Create your patient account'}</h1>
          <p className="mt-4 max-w-lg text-slate-600 dark:text-slate-300">
            Book consultations, track appointment history, and use the AI assistant to get grounded clinic guidance.
          </p>
          <div className="mt-8 space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <p>• Secure JWT authentication</p>
            <p>• Patients, doctors, and admins</p>
            <p>• Protected appointment workflows</p>
            <p>• Modern healthcare UI</p>
          </div>
        </div>
        ...
        <div className="glass-panel rounded-[2rem] p-8">
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            ...
            <input value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="Email" type="email" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-clinic-500 dark:border-slate-800 dark:bg-slate-950" />
          ...
          <div className="mt-6 text-sm text-slate-500 dark:text-slate-400">
            Demo credentials after seeding: patient@demo.com / Patient@123, doctor@demo.com / Doctor@123, admin@demo.com / Admin@123.
          </div>
          <div className="mt-6 text-sm">
            <Link className="text-clinic-600 hover:underline dark:text-clinic-300" to="/">
